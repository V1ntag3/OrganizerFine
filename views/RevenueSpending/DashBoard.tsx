...
function DashBoard({ route, navigation }: any): JSX.Element {
    ...
    const renderHeaderFlat = () => {
        return (
            <>
                <TouchableOpacity onPressIn={() => { setShow(!show) }}>
                    <View style={
                        {
                            position: 'absolute',
                            flexDirection: 'row',
    ...
    const getData = async (monthCurrent, yearCurrent) => {
        try {

            listRevenueSpendings(monthCurrent, yearCurrent).then(async (json: any) => {
                setShow(false)
                    if (json.length > 0) {
                        var array: any = []
                        var gastos: number = 0
                        ...
                        for (var elem in json) {
                            switch (json[elem].category) {
                                case 0:
                                    item0 = + json[elem].value
                                    break
                                case 1:
                                    item1 = + json[elem].value
                                    break
                                case 2:
                                    item2 = + json[elem].value
                                    break
                                case 3:
                                    item3 = + json[elem].value
                                    break
                                case 4:
                                    item4 = + json[elem].value
                                    break
                                case 5:
                                    item5 = + json[elem].value
                                    break
                            }

                            if (json[elem].type == 0) {
                                receitas += json[elem].value
                            }

                            if (json[elem].type == 1) {
                                gastos += json[elem].value
                            }
                            json[elem].realDate = json[elem].created_at
                            json[elem].value = json[elem].value.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })
                            var valor = Number(new Date()) - Number(new Date(json[elem].created_at))
                            json[elem].created_at = json[elem].created_at
                            json[elem].created_at = String(valor / (1000 * 60 * 60 * 24))[0] + ' dias atrás'
    ...
    const handleDateSelect = (selectedDate: String) => {
        ...
        getData(parseInt(selectedDate.toString().slice(4, 7)), parseInt(selectedDate.slice(0, 4)))
        setShow(false)
