...
export const paramsRevenueSpendings = async () => {
    ...
    return new Promise<{ minMonth: string; maxMonth: string }>((resolve, reject) => {
        db.transaction(tx => {
            ...
            tx.executeSql(
                ...
                [],
                (_, { rows: { raw } }) => {
                    const minMonth = raw()[0]?.minMonth ?? moment().format('YYYY-MM');
                    ...
                    tx.executeSql(
                        ...
                        [],
                        (_, { rows: { raw } }) => {
                            const maxMonth = raw()[0]?.maxMonth ?? moment().format('YYYY-MM');
...
export const updateRevenueSpending = async (revenueSpending: { id: string, value: number; about: string; category: number | null; type: number; }) => {
    const db = await connectToDatabase()
...
export const deleteRevenueSpending = async (id: string) => {
    ...
    return new Promise((resolve, reject) => {
        db.transaction(tx => {
            tx.executeSql(
                `UPDATE RevenueSpendings SET deleted_at = datetime('now', '-3 hours') WHERE id = ?`,
                [id],
                (_, result) => resolve(result),
                (_, error) => reject(error)
            );
    ...
    return new Promise((resolve, reject) => {
        db.transaction((tx) => {
            tx.executeSql(
              ...
              FROM RevenueSpendings WHERE deleted_at IS NULL`,
                [],
                (_, result) => {
